...
def process_data(df, tmp):
    ...
    desc_col = df[
            df.columns[
                df.columns.str.contains("Description|<Additional Info>")
                ]
            ]
...
def read_statements(statements, path):
    dataframes = []
...
def more_processing_on_master(master):
    ...
    # calculate Months
    master["Month"] = master["Date"].dt.month.map(
...
def move_completed_files(completed_folder, statements):
...
def main():
    ...
    while True:
        try:
            ...
            completed_folder = (
                    f"/home/{user}/Documents/bank statements/"
                    "completed/"
                    )
            statements = os.listdir(path)

            # ensure filenames matching the CSV file produced by this script,
            # and lock files (produced when a file is open in libreoffice) are
            ...
            # ensure there is at least one valid statement to process or quit
            if len(statements) == 0:
                print(
    ...
    # Make a list of dataframes from the statements
    dataframes = read_statements(statements, path)

    # initalize a temporary dataframe
    tmp = pd.DataFrame(columns=columns.keys())
    tmp = tmp.astype(columns)

    # process transactions in each dataframe and add to master
    for df in dataframes:
        completed_transactions = process_data(df, tmp)
        master = pd.concat([master, completed_transactions])
        print(".", end="")

    # make date, month, quarters, years
    master = more_processing_on_master(master)
    ...
    move_completed_files(completed_folder, statements)
